...
import { Search, MapPin, Phone, ShieldAlert, Award, Star, Compass } from 'lucide-react';
...
export default function LocateDonors() {
  ...
  const handleSearch = async (e?: React.FormEvent, customLat?: number, customLng?: number, customCity?: string) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/ai/match', {
        method: 'POST',
      ...
      console.warn("FastAPI backend offline, matching simulation active.");
      setDonors([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12 flex-1 flex flex-col gap-8 w-full bg-[#FDFBF7]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-stone-900">Locate Blood Donors</h1>
          <p className="text-stone-600 mt-2">Use our smart locator to find verified matching blood donors in your vicinity.</p>
        </div>
        <button 
      ...
      <form onSubmit={(e) => handleSearch(e)} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white border border-stone-200/60 p-6 rounded-2xl shadow-sm shadow-stone-100">
        <div className="flex flex-col gap-2">
          <label className="text-xs font-semibold text-stone-500 uppercase tracking-wider">Blood Group Required</label>
          <select 
            value={bloodType}
        ...
        <div className="flex flex-col justify-end">
          <button 
            type="submit"
            className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl py-3 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all duration-300 shadow-sm shadow-red-600/10"
          >
            <Search className="w-5 h-5" /> Search & Match Donors
          </button>
        </div>
      </form>
      ...
      {loading && (
        <div className="flex-1 flex flex-col items-center justify-center py-16">
          ...
          <span className="text-sm text-stone-600 mt-4 font-medium">Matching compatible donors...</span>
        </div>
      )}
      ...
      {!loading && searched && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-stone-900">Recommended Donor Matches ({donors.length})</h2>
          ...
          {donors.length === 0 ? (
            <div className="text-center py-16 border border-dashed border-stone-200 rounded-2xl bg-white shadow-sm shadow-stone-50">
              ...
              <p className="text-stone-500 mt-4 font-medium">No matches found for blood type {bloodType} in this location.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {donors.map((donor) => (
                ...
                >
                  {/* Top Badge for high match scores */}
                  {donor.match_score >= 0.9 && (
                    <div className="absolute top-4 right-4 flex items-center gap-1 bg-red-50 border border-red-200 text-red-700 text-2xs font-bold px-2 py-0.5 rounded-full">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-red-50 border border-red-200 flex items-center justify-center font-extrabold text-red-700 text-lg shadow-sm">
                        {donor.blood_type}
                      </div>
                      <div>
                    ...
                    <div className="p-3 bg-stone-50 rounded-xl border border-stone-150 flex items-center gap-2.5">
                      <Award className="w-4 h-4 text-rose-600 flex-shrink-0" />
                  ...
                  <div className="mt-6 flex items-center justify-between border-t border-stone-100 pt-4">
                    ...
                    {donor.phone ? (
                      ...
                      <span className="text-xs text-stone-500 italic">No contact available</span>
                    )}
                  </div>
                </div>
              ))}
